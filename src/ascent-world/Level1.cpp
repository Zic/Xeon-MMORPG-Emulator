...
bool ChatHandler::HandleAnnounceCommand(const char* args, WorldSession *m_session)
{
	...
	char msg[1024];
	string input2;
	input2 = MSG_COLOR_LIGHTBLUE;
	input2 += "[";
	input2 += sWorld.announce_tag + "]";
	if(sWorld.GMAdminTag)
	{
		input2 += MSG_COLOR_LIGHTRED;
	...
	//snprintf(msg, 1024, "[Server Notice]"MSG_COLOR_GREEN"  [|Hplayer:%s|h%s|h]: %s", m_session->GetPlayer()->GetName(), m_session->GetPlayer()->GetName(), args);
...
bool ChatHandler::HandleWAnnounceCommand(const char* args, WorldSession *m_session)
{
	...
	char pAnnounce[1024];
	string input3;
	input3 = MSG_COLOR_LIGHTBLUE;
	input3 +="[";
	input3 += sWorld.announce_tag;
	input3 += "]";
	if(sWorld.GMAdminTag)
	{
		input3 += MSG_COLOR_LIGHTRED;
	...
	else if(!sWorld.NameinWAnnounce) {input3+= ":|r ";}
