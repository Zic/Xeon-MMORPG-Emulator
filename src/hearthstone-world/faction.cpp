...
bool isAttackable(ObjectPointer objA, ObjectPointer objB, bool CheckStealth)// A can attack B?
{
	...
	if( player_objA && player_objB )
	{
		...
		if( objA->HasFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_FEIGN_DEATH) || objB->HasFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_FEIGN_DEATH) )
			return false;
	...
	// same faction can't kill each other (we already passed the ffa pvp/duel checks )
