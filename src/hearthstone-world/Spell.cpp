...
void Spell::HandleAddAura(uint64 guid)
{
	...
	if( m_spellInfo->MechanicsType == 25 && m_spellInfo->Id != 25771) // Cast spell Forbearance
		...
		spellid = 25771;
	else if (m_spellInfo->Id == 34754 && p_caster->HasSpell(47549))//Improved Holy Concentration 3.0.9 on 3.1.0 doesn't exist
		spellid = 47894;
	else if (m_spellInfo->Id == 34754 && p_caster->HasSpell(47551))//Improved Holy Concentration 3.0.9 on 3.1.0 doesn't exist
		spellid = 47895;
	else if (m_spellInfo->Id == 34754 && p_caster->HasSpell(47552))//Improved Holy Concentration 3.0.9 on 3.1.0 doesn't exist
		spellid = 47896;
...
exit:*/
	...
	{
		value += float2int32(1 + 0.22f * u_caster->GetDamageDoneMod(SCHOOL_HOLY) + 0.14f * u_caster->GetAP());
		AuraPointer p = unitTarget->FindActiveAura(31803);
		uint32 stack = p ? p->stackSize : 0;
		if(stack)
	...
	{
		value = float2int32(1 + 0.22f * u_caster->GetDamageDoneMod(SCHOOL_HOLY) + 0.14f * u_caster->GetAP());
		AuraPointer p = unitTarget->FindActiveAura(53742);
		uint32 stack = p ? p->stackSize : 0;
		if(stack)
			value += value * stack / 10;	// increased by 10% for each application
	}
