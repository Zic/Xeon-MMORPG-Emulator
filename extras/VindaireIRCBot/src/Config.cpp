...
bool ConfigFile::SetSource(const char *file, bool ignorecase)
{
	...
	if(file != 0)
	{
		...
		if(!f)
		{
			Log.Notice("Config", "Could not open %s.", file);
			return false;
		}
...
parse:
			...
			if(in_block)
			{
				...
				if(in_multiline_quote)
				{
					...
					{
						Log.Notice("Config", "Quote without variable.");
						return false;
					}
			...
			else
			{
				...
				if(offset != string::npos)
				{
					...
					{
						Log.Notice("Config", "Block without name.");
						return false;
					}
			...
			{
				Log.Notice("Config", "Exception in config parsing.");
				return false;
			}
		...
		if(in_multiline_comment)
		{
			Log.Notice("Config", "Unterminated comment.");
			return false;
		}

		if(in_multiline_quote)
		{
			Log.Notice("Config", "Unterminated quote.");
			return false;
		}
